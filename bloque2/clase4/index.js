...
const maxPrice = (productos = products) => {
    let maximo = 0;
    for (producto of productos) {
        producto.precio > maximo ? maximo= producto.precio : null;
    }

    return maximo;
}
//sacamos el minimo stock
const minimo = (stock) => Math.min(...stock);

const stockMinimo = (productos = products) => {
    const stock =[];
    for (producto of productos) {
        stock.push(producto.stock);
    }
    const minStock = minimo(stock);
    
    return minStock;
}

//muestra el producto que tiene mas colores

const maxColores = (productos = products) => {
   let max = 0;
   let pProducto = 0;

    for (let i = 0; i < productos.length; i++) {
        
        if (productos[i].colores.length >= max) {
            max = productos[i].colores.length;
            
            pProducto = i;
        }  
    }

    document.write(" <br>El producto que tiene mas colores es " + productos[pProducto].nombre + " con " + max + " colores" );
}

//array reversa 
const reversa = (array = products) => {
